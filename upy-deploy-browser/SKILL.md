...
### Phase 1.5: 部署策略与清理（approval_request 安全门）

上传前必须经 `approval_request`（`deploy_strategy_select`）让用户选择部署策略：

| 策略 | 含义 |
|------|------|
| `upload_only` | 不清理设备文件，直接上传当前项目 |
| `clean_then_upload` | 清理旧项目文件/业务目录后上传（**推荐默认**） |
| `erase_then_upload` | 清理设备可列出的全部文件/目录后上传；**必须 dry-run + 二次确认** |
| `save_partial` | 保存当前进度为 checkpoint，暂不上传 |

`erase_then_upload` ≠ 重新烧录 MicroPython 解释器固件——它只清空文件系统中的文件/目录（固件烧录是 `upy-flash-mpy-firmware-browser` 的职责）。

**破坏性清理确认门**（待删列表由 `browser_validate`(`device_clean_plan`) dry-run 产出，`device_command` 执行）：

- `clean_then_upload` → `approval_request`(`confirm_clean_device_project`)：展示 `project_files` dry-run 待删列表，确认后才执行。
- `erase_then_upload` → `approval_request`(`confirm_erase_device_fs`)：展示 `erase_all` dry-run 的完整待删文件/目录列表，**用户必须二次确认**后才执行。
- 清理必须覆盖旧的生产禁止产物：`conf.mpy`、`boot.mpy`、`main.mpy`、`board.mpy` 及 `drivers/**/mock.py`|`mock.mpy`——否则即使新上传过滤正确，设备仍可能 import 到旧文件。
- 日志只在用户确认后才删；默认不静默删除旧日志（可选保留 / 下载 / 清除）。

**运行时依赖安装**：读取 generate 声明的 `manifest_content.runtime_dependencies.mip`，用 `device_command`（mip install）只安装 MicroPython/micropython-lib 包（不把库源码 vendor 进项目），安装后用 `fs ls` 校验目标目录（如 `:lib`、`:lib/unittest`）确实存在。若因网络/代理不可用失败，标记 `runtime_dependency_install_network_unavailable` 提示用户修复网络后重试，**不要误判为 generate 代码错误**。

**可选设备端测试**：上传 + 软复位 + 读日志后，经 `approval_request`(`run_device_tests`) 询问是否运行设备契约测试（`device/tests/test_*.py`）。默认建议运行，但必须允许跳过（部分测试会触碰真实硬件）。
...
`deploy_result_judge` 综合 upload summary、clean result、mip install result、wait/probe、REPL capture、设备日志报告、device tests 结果与用户人工反馈，按下表给出三级判定（`PASS` / `PASS_WITH_WARNINGS` / `FAIL`）。

**硬 FAIL 信号：**

| 信号 | 结果 |
|------|------|
| upload failed / clean failed | `FAIL` |
| mip 依赖安装或导入校验失败 | `FAIL` |
| 禁止产物上传（`:conf.mpy`、`:boot.mpy`、`:drivers/*/mock.py`、`:drivers/*/mock.mpy`）——即使上传工具返回 success | `FAIL` |
| wait/probe failed | `FAIL` |
| REPL 出现 `Traceback` / `Guru Meditation Error` / `rst cause:` / `MemoryError`/`ENOMEM` / `ValueError`/`OSError`/`ImportError`/`AttributeError` | `FAIL` |
| `deploy_log_report.json` 的 `error_count > 0`（REPL 可能因断连丢输出，日志是补充权威源；P0/P1 直接 FAIL，P2 作辅助） | `FAIL` |
| device tests failed | `FAIL` |

**`PASS_WITH_WARNINGS`（关键：REPL 空输出不直接判 FAIL）：** 若 serial 捕获为空，但上传/清理成功、`error_count=0` 且 device tests 未失败，则判 `PASS_WITH_WARNINGS` 并附 warning（如 `serial capture produced no output`）——应用可能只写 rotating file logger，或运行时无 stdout。其余无命中即 `PASS`。
...
**失败原因速查（device 运行错误 → 根因 → 修复方向）：**

| 错误签名 | 根因 | 修复方向 |
|---------|------|---------|
| `ImportError` / `ModuleNotFoundError` | 文件缺失或上传不全 | 检查上传清单、mip 依赖是否装上 |
| `OSError: [Errno 110] ETIMEDOUT` / `OSError -110` | I2C/总线接线或地址错误 | 核对接线与器件地址 |
| `RuntimeError: No I2C device found` | 器件未应答 | 检查供电/接线/上拉 |
| `RuntimeError: WiFi ...` / 连接超时 | 凭证占位符未替换或网络不可用 | 替换 conf 中的 SSID/密码占位符、检查网络 |

PASS / PASS_WITH_WARNINGS → 流程结束，经 `approval_request`(`deploy_result_feedback`) 展示串口/设备、策略、清理结果、upload summary、reset/wait、REPL 摘要、日志报告、device tests 结果，并收集用户可选反馈（实际现象、报错、手动观察）。
FAIL → 经 `approval_request`(`deploy_fail_next_action`) 展示同样诊断摘要，允许进入 `upy-autofix-browser` / `upy-generate-browser(mode=fix)` / 保存 checkpoint；进入修复时必须随 `phase_complete` 携带完整 **`error_context`**（REPL 输出 + `deploy_log_report.json` + `deploy_logs/*.log` + 用户反馈）。
