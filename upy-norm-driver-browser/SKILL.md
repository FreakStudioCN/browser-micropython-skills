---
name: upy-norm-driver-browser
description: Use when normalizing/standardizing an existing MicroPython driver .py file (non-main.py) to the GraftSense coding spec inside Blockless Web Builder. Triggers like "normalize this driver", "规范化这个驱动文件", "按规范改写", or a provided .py driver path needing standardization.
---
...
Normalize an existing-but-nonstandard MicroPython driver `.py` file (non-`main.py`) to the GraftSense coding spec and return the fully rewritten file. The rewrite is performed by the LLM applying the P0→P2 rule checklist below; the driver's public API names, method-signature semantics, business logic, and hardware timing are never changed. Blockless Web Builder is the only target runtime for this browser skill.
...
- The project-store path of the driver `.py` to normalize.

## Outputs

- The fully rewritten driver `.py`, written back to the Blockless project store after user approval.
- A short rewrite-summary listing which P0/P2 items were applied.
- `phase_complete` for `upy-norm-driver` with `status`, `evidence`, `artifacts`, and a recoverable `next_action` when needed.

## Blockless Primitive Sequence

1. `file_operation`: read the target driver `.py` (full content — never a session cache).
2. `browser_validate` (`python_syntax`): confirm the source parses before rewriting.
3. Apply the P0→P2 rewrite rules below (LLM-driven; see the domain/validate boundary section).
...
6. `file_operation`: on approval, overwrite the original driver file in the project store.
...
本仓库已附带完整规范文档（22 章、2200+ 行），按相对路径查阅：
