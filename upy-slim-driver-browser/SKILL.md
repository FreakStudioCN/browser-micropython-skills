...
description: Use when reducing the RAM footprint of any MicroPython .py file (driver, main.py, etc.) to the GraftSense memory guide inside Blockless Web Builder. Triggers like "减少内存占用", "slim", "降低RAM使用", "优化内存", or a .py path/dir provided for memory reduction.
---
...
Reduce the RAM footprint of any MicroPython `.py` file per the GraftSense memory-minimization guide (the P0→P2 checklist below). Optimization is performed by the LLM applying the rules; the file's public API and behavior are preserved. Blockless Web Builder is the only target runtime for this browser skill.
...
- The project-store path of the `.py` file (or directory, for multi-file optimization) to slim.

## Outputs

- The slimmed `.py` file(s), written back to the Blockless project store after per-file user approval.
- An optimization summary listing which P0/P1/P2 items were applied.
- `phase_complete` for `upy-slim-driver` with `status`, `evidence`, `artifacts`, and a recoverable `next_action` when needed.
...
3. Apply the P0→P2 memory-reduction rules below (LLM-driven; see the domain/validate boundary section).
...
Optimization is the LLM applying the memory rules below (`_CONST`, `__slots__`, bounded `gc.disable()` sections, etc.). `browser_validate` performs only the objective subset — does it still parse (`python_syntax`), is the path valid (`project_files`). It does **not** apply the optimizations. Blockless Web Builder runs both.
...
[内存优化指南（本仓库附带）](../docs/reference/MicroPython_Memory_Footprint_Minimization_Guide.md)
