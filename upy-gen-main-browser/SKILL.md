---
name: upy-gen-main-browser
description: Use when generating a new main.py test file from scratch for a MicroPython driver inside Blockless Web Builder. Triggers like "generate main.py", "生成测试文件", "从零生成main.py", or a driver .py provided to create a test program.
---

# upy-gen-main-browser

## Purpose

Generate a complete, spec-compliant `main.py` test file from scratch by analyzing a driver's public API (full coverage across the chip's API dimensions, three test scenarios). Generation is performed by the LLM applying the rules below. Blockless Web Builder is the only target runtime for this browser skill.
...
This browser contract preserves the source skill's responsibility, generation rules, required evidence, and failure semantics. Source-side local file read/write and "确认写入" prompts are replaced by Blockless primitives only:
- `file_operation`
- `approval_request`
- `browser_validate`
- `phase_complete`
...
- The project-store path of the driver `.py` to generate a test file for.

## Outputs

- The generated `main.py`, written to the Blockless project store after user approval.
- An API-coverage summary.
- `phase_complete` for `upy-gen-main` with `status`, `evidence`, `artifacts`, and a recoverable `next_action` when needed.

## Blockless Primitive Sequence

1. `file_operation`: read the driver `.py` (full content).
2. Apply the generation rules below (LLM-driven; see the domain/validate boundary section).
3. `browser_validate` (`python_syntax`, `project_files`): objective checks on the generated file and its path.
4. `approval_request` + `file_operation`: confirm and write `main.py` to the project store.
...
- Return `failed` when the driver file is missing/malformed or post-generation validation reports blocking errors.
...
Generation is the LLM applying the rules below. `browser_validate` performs only the objective subset — does it parse (`python_syntax`), is the path project-relative (`project_files`). It does **not** replace the coverage rules. Blockless Web Builder runs both.
...
本 Skill 的生成规则基于 GraftSense 驱动编写规范文档（22 章、2200+ 行），已随本仓库附带，按相对路径查阅：
