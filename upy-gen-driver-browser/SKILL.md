...
description: Generate a MicroPython driver from a PDF datasheet or Arduino/C++ code inside Blockless Web Builder, when upypi and GitHub have no driver. Flow: extract → debug version → hardware-verify loop → strip debug → normalize. Triggers when upy-analyze-browser finds no driver, or invoked directly.
---
...
Generate a normalized MicroPython driver from non-MicroPython sources (PDF datasheet, Arduino/C++ code, or chip model) via: extract → generate debug version → hardware-verify loop → strip debug → normalize. An independent skill, callable by `upy-analyze-browser`, `upy-autofix-browser`, or the user directly. The hard ordering constraint (Step 3 hardware verification must complete before Step 4) is preserved. Blockless Web Builder is the only target runtime for this browser skill.
...
This browser contract preserves the source skill's responsibility, extraction/verify/strip flow, hard ordering constraint, and failure semantics. Source-side PDF/Arduino extraction scripts, repo cloning, and device-CLI hardware verification are replaced by Blockless primitives only:
- `file_operation`
- `device_command`
- `browser_validate`
- `approval_request`
...
- One of: PDF datasheet, Arduino/C++ source, Arduino GitHub repo URL, or chip model.
- A user-granted Blockless device session for the Step 3 hardware-verify loop.
- Validation inputs for: `doc_extract_pdf`, `arduino_convert`, `python_syntax`, `generate_quality`.

## Outputs

- artifacts/{chip}_understanding.json (or {chip}_mapping.json), the debug-version driver, and the final normalized `firmware/drivers/{chip}_driver/{chip}.py`.
- `phase_complete` for `upy-gen-driver` with `status`, `evidence`, `artifacts`, and a recoverable `next_action` when needed.

## Blockless Primitive Sequence

1. `browser_validate` (`doc_extract_pdf` / `arduino_convert`): extract text/structure from the source (returns `partial` until the provider is loaded).
2. Generate the debug-version driver per the rules below (LLM-driven; see the domain/validate boundary section).
3. `device_command`: run the Step 3 hardware-verify loop on the bound board (scan → run → analyze → edit), max 10 rounds.
4. After `SELF_TEST_PASS`, strip debug and normalize (hand off to `upy-norm-driver-browser`).
5. `browser_validate` (`python_syntax`, `generate_quality`): validate the final driver.
...
- Missing a device for Step 3 returns a partial envelope and **must pause** rather than skip hardware verification:

```json
{
  "status": "partial",
  "phase": "upy-gen-driver",
...
- Return `failed` when extraction fails, or the verify loop exceeds 10 rounds without `SELF_TEST_PASS`.
- Return `partial` when the extraction provider, device session, or USB permission is missing.
- Include `capability_required` (`browser_validate.<kind>` / `device_command.<action>`) and `next_action` (`load_provider`/`connect_device`/`grant_usb_permission`).
- Do not bypass Blockless primitives for local execution paths.

## Domain Generation vs browser_validate (boundary)

Understanding the datasheet/Arduino source and writing the driver is the LLM's job; extraction and hardware run go through `browser_validate` (`doc_extract_pdf`/`arduino_convert`) and `device_command`. `browser_validate` does **not** translate or design the driver. Blockless Web Builder runs both.
...
- 已授权 Blockless 设备会话（`device_command`，硬件验证阶段需要）
- 已加载 Blockless 文档/转换 provider（`browser_validate` 的 `doc_extract_pdf` / `arduino_convert`）
...
├─ GitHub Arduino 仓库 URL       → browser_validate doc_fetch → Step 1B
...
用 `browser_validate` 的 `doc_extract_pdf` 提取 PDF 文本（provider 未加载时返回 `partial`），输出 `{chip}_text.json`：
...
用 `browser_validate` 的 `arduino_convert` 做 API 映射 + 结构提取（provider 未加载时返回 `partial`），输出 `{chip}_mapping.json`：
...
用 `device_command`（scan）枚举已授权的 Blockless 设备：
...
用 `device_command`（resume run）把 `firmware/drivers/{chip}_driver/{chip}_debug.py` 通过 REPL 送入执行，不写 flash，秒级反馈（而非 fs cp）。
...
| 设备崩溃无响应 | 代码导致 crash | `device_command`（soft_reset）→ 下一轮 |
...
| # | 条件 | 验证方式 |
|---|------|----------|
| 1 | Step 3 硬件验证循环已执行 | 最近一次 `device_command`（resume run）有输出 |
...
upy-gen-driver (本 skill)
    ├── browser_validate doc_extract_pdf   ← PDF 文本提取
    ├── browser_validate arduino_convert   ← Arduino API 映射
    ├── device_command (resume run)        ← 硬件验证（webserial-device-interaction-browser）
    └── upy-norm-driver-browser            ← 规范化
    ↓
输出: firmware/drivers/{chip}_driver/{chip}.py
    ↓
...
- **验证用 `device_command`（resume run）**：不改动 flash，快速迭代
