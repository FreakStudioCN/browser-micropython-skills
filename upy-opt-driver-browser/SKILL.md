---
name: upy-opt-driver-browser
description: Use when optimizing the execution speed of any MicroPython .py file (driver, main.py, etc.) to the GraftSense performance guide inside Blockless Web Builder. Triggers like "优化性能", "optimize", "加速", or a .py path/dir provided for performance improvement.
---

# upy-opt-driver-browser

## Purpose

Optimize the execution speed of any MicroPython `.py` file per the GraftSense performance guide (the P0→P2 optimization checklist below). Optimization is performed by the LLM applying the rules; the file's public API and behavior are preserved. Blockless Web Builder is the only target runtime for this browser skill.
...
This browser contract preserves the source skill's responsibility, optimization rules, required evidence, and failure semantics. Source-side local file read/write and "确认写入" prompts are replaced by Blockless primitives only:
- `file_operation`
- `approval_request`
- `browser_validate`
- `phase_complete`
...
- The project-store path of the `.py` file (or directory, for multi-file optimization) to speed up.

## Outputs

- The optimized `.py` file(s), written back to the Blockless project store after per-file user approval.
- An optimization summary listing which P0/P1/P2 items were applied.
- `phase_complete` for `upy-opt-driver` with `status`, `evidence`, `artifacts`, and a recoverable `next_action` when needed.

## Blockless Primitive Sequence

1. `file_operation`: read the target `.py` (full content).
2. `browser_validate` (`python_syntax`): confirm the source parses.
3. Apply the P0→P2 optimization rules below (LLM-driven; see the domain/validate boundary section).
4. `browser_validate` (`python_syntax`, `project_files`): objective post-checks.
5. `approval_request` + `file_operation`: confirm per file and write back.
...
- Return `failed` when the target file is missing/malformed or post-optimization validation reports blocking errors.
...
## Domain Optimization vs browser_validate (boundary)

Optimization is the LLM applying the rules below (including documenting `@viper` overflow risk, `@native` limits, RP2040-only SIO notes). `browser_validate` performs only the objective subset — does it still parse (`python_syntax`), is the path valid (`project_files`). It does **not** apply the optimizations. Blockless Web Builder runs both.
...
[性能优化指南（本仓库附带）](../docs/reference/MicroPython_Performance_Optimization_Guide.md)

[驱动规范文档（本仓库附带）](../docs/reference/upy_driver_dev_spec_summary.md)
